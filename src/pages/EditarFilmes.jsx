...
export default function EditarFilmes() {
  ...
  const idFilme = searchParams.get("filmeId");

  useEffect(() => {
    async function carregarFilme() {
      if (!idFilme) {
        setError("Filme não especificado.");
        setLoading(false);
        return;
      }

      try {
        const docRef = doc(db, "conteudos", idFilme);
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      navigate("/cadastro"); // ou onde quiser voltar
    } catch (err) {
      alert("Erro ao atualizar: " + err.message);
    }
  }

  if (loading) return <div style={{color:"white", padding: 30}}>Carregando...</div>;
